...
struct aws_dynamo_v2_delete_item_response *aws_dynamo_parse_v2_delete_item_response(const char *response, int response_len)
{
    ...
    struct aws_dynamo_v2_delete_item_response *delResponse = NULL;
    delResponse = calloc(1, sizeof(*(delResponse)));
    if (delResponse == NULL) {
        Warnx("aws_dynamo_parse_v2_delete_item_response: alloc failed.");
        return NULL;
    }
    ...
    if (root == NULL) {
        free(delResponse);
        if (strlen(errbuf)) {
            Warnx("aws_dynamo_parse_v2_delete_item_response: json parse failed, '%s'", errbuf);
        } else {
            Warnx("aws_dynamo_parse_v2_delete_item_response: json parse failed, 'unknown error'");
        }
        return NULL;
    }
    ...
    if (capacityUnit != NULL) {
        delResponse->consumedCapacityDel.capacityUnits = YAJL_GET_INTEGER(capacityUnit);
        ...
        delResponse->consumedCapacityDel.tableName = strdup(tblName);
    }

    getObjectItem(root, &delResponse->item, AWS_DYNAMO_JSON_ATTRIBUTES);
...
struct aws_dynamo_v2_delete_item_response *aws_dynamo_v2_delete_item(struct aws_handle *aws, const char *request)
{
    ...
    int response_len = 0;
    struct aws_dynamo_v2_delete_item_response *r = NULL;

    if (aws_dynamo_request(aws, AWS_DYNAMO_V2_DELETE_ITEM, request) == -1) {
        return NULL;
    }
    ...
    if (response == NULL) {
        Warnx("aws_dynamo_v2_delete_item: Failed to get response.");
        return NULL;
    }

    if ((r = aws_dynamo_parse_v2_delete_item_response(response, response_len)) == NULL) {
        Warnx("aws_dynamo_v2_delete_item: Failed to parse response: '%s'", response);
        return NULL;
    }
...
void aws_dynamo_dump_v2_delete_item_response(struct aws_dynamo_v2_delete_item_response *r) {
    ...
    Debug("consumed_capacity_units = %f", r->consumedCapacityDel.capacityUnits);
    Debug("TableName = %s", r->consumedCapacityDel.tableName);
