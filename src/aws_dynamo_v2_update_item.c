...
struct aws_dynamo_v2_update_item_response *aws_dynamo_parse_v2_update_item_response(const char *response, int response_len)
{
    ...
    struct aws_dynamo_v2_update_item_response *updResponse = NULL;
    updResponse = calloc(1, sizeof(*(updResponse)));
    if (updResponse == NULL) {
        Warnx("aws_dynamo_parse_v2_update_item_response: alloc failed.");
        return NULL;
    }
    ...
    if (root == NULL) {
        if (strlen(errbuf)) {
            Warnx("aws_dynamo_v2_update_item_response: json parse failed, '%s'", errbuf);
        } else {
            Warnx("aws_dynamo_v2_update_item_response: json parse failed, 'unknown error'");
        }
        return NULL;
    }
    ...
    if (capacityUnit != NULL) {
        updResponse->consumedCapacityUpd.capacityUnits = YAJL_GET_INTEGER(capacityUnit);
        ...
        updResponse->consumedCapacityUpd.tableName = strdup(tblName);
    }

    getObjectItem(root, &updResponse->item, AWS_DYNAMO_JSON_ATTRIBUTES);
...
struct aws_dynamo_v2_update_item_response *aws_dynamo_v2_update_item(struct aws_handle *aws, const char *request)
{
    ...
    int response_len = 0;
    struct aws_dynamo_v2_update_item_response *r = NULL;

    if (aws_dynamo_request(aws, AWS_DYNAMO_V2_UPDATE_ITEM, request) == -1) {
        return NULL;
    }
    ...
    if (response == NULL) {
        Warnx("aws_dynamo_v2_update_item: Failed to get response.");
        return NULL;
    }

    if ((r = aws_dynamo_parse_v2_update_item_response(response, response_len)) == NULL) {
        Warnx("aws_dynamo_v2_update_item: Failed to parse response: '%s'", response);
        return NULL;
    }
...
void aws_dynamo_dump_v2_update_item_response(struct aws_dynamo_v2_update_item_response *r) {
    ...
    Debug("consumed_capacity_units = %f", r->consumedCapacityUpd.capacityUnits);
    Debug("TableName = %s", r->consumedCapacityUpd.tableName);
